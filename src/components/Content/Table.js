...
export default class Tabels extends Component{ 
    ...
    remove(e){
        ...
        this.setState({ title: "删除文章", text: "确定以管理员身份删除该文章？", visible: true });
    }
    ...
    ok_modal(){  //删除
        if(typeof this.index === "number" && Number.isNaN(this.index) === false){
            ...
            axios.post("/admin/posts/remove",{ 
                ...
                this.setState({ table: data.table, page: page_index, visible: false, title: null, text:null });
            }).catch(({response})=>{
                message.error(response.data.err);
            });
    ...
    getTable(path, page, state=2){
        axios.get(`/admin/table/${path}?page=${page}&state=${state}`).then(({ data })=>{
            this.setState({ table: data.table, count: data.count, page: page, loading: false, update: false });
        }).catch(({ response })=>{
            message.error(response.data.err);
        });
    }
    ...
    filter(pagination, filter, sort){
        if(this.state.filter[0] !== filter.state[0]){
            ...
            axios.get(`/admin/table/posts?page=1&state=${state}`).then(({ data })=>{
                this.setState({ table: data.table, count: data.count, page: 1, filter: filter.state, loading: false, update: false });
            }).catch(({ response })=>{
                message.error(response.data.err);
            });
        }
    ...
    render(){
        ...
        const posts_columns = [
            ...
            },
            {
                ...
                render: function(title, record){
                    return <Link to={`/p/${record._id}?state=${record.state}`}>{title}</Link>
                }
            },
            {
                title: '作者',
            ...
            },
            {
                ...
                render: function(state){
                    switch(state){
                        case 0:
                        return <Tag color="blue" key="state0">待审核</Tag>;

                        case 1:
                        return <Tag color="red" key="state1">已拒绝</Tag>;

                        case 2:
                        return <Tag color="green" key="state2">已通过</Tag>;
                    }
                }
            },
            {
                    ...
                    return( 
                        <div>
                            <Tag color="volcano" key={"taga"}>
                                <span onClick={this.remove} data-index={data.index}>删除</span>
                            </Tag>
                            <Tag color="#ea6f5a" visible={data.state===0 || data.state==2} key={"tagb"}>
                                <span onClick={this.reject} data-index={data.index}>拒绝</span>
                            </Tag>
                            <Tag color="#2db7f5" visible={data.state!==2} key={"tagc"}>
                                <span onClick={this.resolve} data-index={data.index}>通过</span>
                            </Tag>
                        </div>
                    );
                }
        ...
        const columns = this.props.match.params.type === "user" ? user_columns : posts_columns;
    	return(
            <div className="content">
                ...
                <div className="table_page"><Pagination current={page} total={count*10} onChange={this.setPage}/></div>
                <Modal 
                    title={title} 
                    visible={visible} 
                    onOk={this.ok_modal} 
