...
export default class Create extends Component {
    ...
    uploadFile(key, file) {
        if (/^image\//.test(file.type) || file.size < 2000000) {
            ...
            const err = file.size > 2000000 ? "请上传低于2M的图片" : "上传文件不是图片格式";
            message.error(err);
    ...
    create(e) {
        e.stopPropagation();
        Promise.resolve(this.state).then(({ ...data }) => {
            ...
            if (/^.{1,30}$/.test(type) && /^.{1,30}$/.test(describe)) {
                    ...
                    return this.is_update ? axios.post("/admin/label/update", update) : axios.post("/admin/label/create",create);
                } 
                else {
    ...
    render() {
        ...
        const path = this.props.location.hash !== "" ? `/label/create${this.props.location.hash}` : "/label/create";
        return (
                    ...
                    <ul>
                        <li className="label_image">
                            ...
                            <div>
                                ...
                                <button onClick={this.openFile} type="button">上传图片</button>
                                <input
                                    type="file"
                                    name="file"
                                    style={{ display: "none" }}
    ...
    componentDidMount() {
        if (this.props.location.hash !== "") {
            ...
            axios.get(`/admin/label?type=${type}&page=1`).then(({ data }) => {
